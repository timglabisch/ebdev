use crate::ui::types::{CompletedStage, TaskInfo, TaskState};
use ratatui::prelude::*;
use ratatui::widgets::{Block, Borders, Paragraph};

pub fn draw_header(frame: &mut Frame, area: Rect, task_name: &str, tasks: &[TaskInfo], completed_stages: &[CompletedStage]) {
    ...
    // Add counts from completed stages
    let stages_completed: usize = completed_stages.iter().map(|s| s.task_count() - s.failed_count).sum();
    let stages_failed: usize = completed_stages.iter().map(|s| s.failed_count).sum();

    let total_completed = completed + stages_completed;
    let total_failed = failed + stages_failed;
    let total = tasks.len() + completed_stages.iter().map(|s| s.task_count()).sum::<usize>();
